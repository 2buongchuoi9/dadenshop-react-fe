  ...
  return (
      ...
      render={() => (
        <div className={cl('search-result')} tabIndex="-1">
          <WrapperPopper>
            ...
            {resultSearch && resultSearch.map((result) => <ProductItem key={result.id} data={result} />)}
          </WrapperPopper>
        </div>
      )}
    ...
    >
      <div className={cl('search')}>
        ...
        {!loading && valueSearch && (
          <Button iconLeft={<TiDelete className={cl('icon')} />} className={cl('clear')} onClick={handleClear} />
        )}
